# -*- coding: utf-8 -*-
"""
Created on Wed Jul 30 11:18:21 2025
...
if optim == "Y":
    ...
    plt.title(f"Control Pulse Shape: Centre:{opt_center:.5f}, Width: {opt_width:.5f}, $\\beta$: {opt_beta:.5f}, $\\Omega_d$: {opt_omega_d:.3f}")
    plt.legend(loc='best')
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    plt.title(f"Fidelity vs Gate Duration\n(Width: {opt_width:.3f}, $\\beta$: {opt_beta:.3f}, $\\alpha$: {opt_alpha:.3f}, $\\Omega_d$: {opt_omega_d:.3f})")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()

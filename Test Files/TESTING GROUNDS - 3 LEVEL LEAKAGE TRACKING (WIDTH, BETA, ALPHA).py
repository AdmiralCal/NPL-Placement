# -*- coding: utf-8 -*-
"""
Created on Wed Jul 30 10:53:19 2025
...
Now includes optimization over width, beta, and anharmonicity alpha.
"""
...
# -----------------------------
def gate_fidelity(width, beta, alpha, times):
    center = 3 * width
    psi0 = np.array([[1], [0], [0]], dtype=complex)
...
def objective2(params):
    width, beta, alpha = params
    center = 3 * width
    F_avg = gate_fidelity(width, beta, alpha, times)
    fidelity_loss = -F_avg
...
def stop_if_high_enough(current_guess):
    iter_count[0] += 1
    width, beta, alpha = current_guess
    fid = gate_fidelity(width, beta, alpha, times)
    best_fid[0] = fid
...
# OPTIMIZATION CALL
# -----------------------------
initial_guess = [width, beta, alpha]
bounds = [(0.1, t_total / 2), (-3.0, 3.0), (-3.0, 0.0)]
if optim == "Y":
    ...
    opt_width, opt_beta, opt_alpha = result.x
    ...
    print(f"Optimal alpha:     {opt_alpha:.5f}")
    print(f"Maximum fidelity:  {max_fidelity:.10f}")

    psi_t = unitary_evolution_3level(H_func_3level, times, psi0,
                                     delta, opt_alpha, omega_d, shape, opt_center, opt_width, I, Q, opt_beta)
    ...
    durations = np.linspace(0.1, t_total, 100)
    fidelities = []
    for duration in durations:
        time_grid = np.linspace(0, duration, n_steps)
        F = gate_fidelity(opt_width, opt_beta, opt_alpha, time_grid)
        fidelities.append(F)
    ...
    plt.title(f"Fidelity vs Gate Duration\n(Width: {opt_width:.3f}, $\\beta$: {opt_beta:.3f}, $\\alpha$: {opt_alpha:.3f})")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
